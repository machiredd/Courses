...
def question_4(systems,coordinates,reverse_map):
    ...
    ### Compute adjacency matrix
    distance_matrix = [[euclidean_dist(coordinates[i], coordinates[j]) for i in systems] for j in systems]
    visited, vertices = search4(systems, distance_matrix,reverse_map)

#    visited, vertices = dijkstra_4(systems, distance_matrix,reverse_map)
    ...
    final_min_dist = 0
    for v,v_d in enumerate(vertices):
        if vertices[v].d > final_min_dist:
            final_min_dist = vertices[v].d
            longestv = vertices[v].pi
    print("Longest distance that must be traveled to reach all stars is %11.4e" % final_min_dist)

    v = longestv.identifier
    while vertices[v].pi != None:
        u = vertices[v].pi.identifier
        if distance_matrix[u][v] == final_min_dist:
            print("Longest distance is between %s and %s" %(reverse_map[u],reverse_map[v]))
            start = u
            finish = v
        v=u
...
#
#def dijkstra_4(systems, graph, mapping) -> (float, List[int]):
#    # initialization of the nodes
#    source = 0
#    vertices = [Vertex(index) for index, _ in enumerate(graph)]
#    vertices[source].d = 0
...
#    
#    pq = PriorityQueue_update(Q.queue)
#    while len(S) < len(systems):
#        d,_,u = pq.pop_task()
#        S.add(u)
#        for adj_star in systems:
#            if adj_star not in S:
#                if vertices[adj_star].d > max(vertices[u].d, graph[u][adj_star]):
#                    vertices[adj_star].d = max(vertices[u].d, graph[u][adj_star])
#                    vertices[adj_star].pi = vertices[u]
#                    pq.remove_task(adj_star)
#                    pq.add_task(adj_star,max(vertices[u].d, graph[u][adj_star]))
#
#    return S, vertices
...
def search4(systems, graph: List[List[int]],mapping) -> List[int]:
    ...
    while queue != []:
        ...
        if remaining_nodes != []:
            ...
            vertices[minpos].d = min_dist
            vertices[minpos].pi = vertices[u]
...
def q1_shortest_path(start: str, destination: str) -> List[str]:
    ...
    print(route)
    if dist == 0:
        print('There exists no path from',start,'to', destination)
    return route
...
#    q1_shortest_path("6VDT-H", "Dodixie")
    q1_shortest_path("U8MM-3","TP7-KE")
...
#    q2_best_path("Egmur", "Javrendei")
    q2_best_path("6VDT-H", "Rancer")


def question3():
#    max_distance = 1.0e+16
    max_distance = 2.468e+16
